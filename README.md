...
* **Catalog Dispersion & Localized Era Anomalies (Figure 6):** Evaluates track popularity across `album_families`, uncovering three distinct structural and statistical dynamics:
  ...
  * **Intra-Era Re-recording Variance:** `album_families` containing re-recordings (*Taylor's Versions*) alongside original releases demonstrate significantly wider Interquartile Ranges (IQR) compared to single-release eras, as the decade-long gap between original masters and re-recorded editions splits tracks into distinct popularity tiers.
