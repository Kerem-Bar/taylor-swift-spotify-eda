...
* **Catalog Dispersion & Localized Era Anomalies (Figure 6):** Evaluates track popularity across `album_families`, uncovering three distinct structural and statistical dynamics:
  * **Inter-Era Temporal Decay:** Newer, contemporary releases like `TTPD` (2024) display tight clustering and high baseline popularity, whereas older standalone eras exhibit wider box plot dispersion as time separates long-term catalog hits from deep cuts.
 * **Catalog Dispersion & Localized Era Anomalies (Figure 6):** Evaluates track popularity across `album_families`, uncovering three distinct structural and statistical dynamics:
  * **Inter-Era Temporal Decay:** Newer, contemporary releases exhibit significantly lower variance and narrower Interquartile Ranges (IQR), whereas older standalone eras demonstrate higher variance and wider Interquartile Ranges (IQR) as time separates long-term catalog hits from deep cuts.
  * **Intra-Era Re-recording Variance:** `album_families` containing multiple album editions (re-recordings alongside original releases) display significantly higher variance and wider Interquartile Ranges (IQR) compared to single-release eras, driven by the popularity gap between original masters and re-recorded versions.
  * **Localized Anomalies vs. Global Baselines:** Demonstrates that flat global baselines (Mean = 57.86, Median = 62.00) completely mask era-specific behavior, whereas localized screening isolates true micro-anomalies:
    ...
    * **`TTPD` (2024):** Localized fences (68.50 to 88.50) isolated anomalies on both ends of a highly streamed era: *"Fortnight"* (popularity: 91) above the upper fence, and *"Clara Bow"* (popularity: 67) as an underperforming deep-cut anomaly below the lower fence, despite sitting well above global baselines.
