...
* **Localized Era & Temporal Framework (Preventing Pooling Bias):** `popularity` remains the core target variable. While `release_date_D` was engineered specifically for high-resolution visualization placement (Figure 5), analytical modeling requires synthesizing the active numerical time feature (`release_year`) directly with `album_families`. Downstream ML algorithms must evaluate target behavior dynamically across this dual time-era structure to prevent global pooling bias from masking localized anomalies (as demonstrated in Figure 6 with *evermore* and *TTPD*).
---
